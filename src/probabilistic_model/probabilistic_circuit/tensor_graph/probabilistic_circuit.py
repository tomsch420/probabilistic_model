...
class Layer(ABC):
    """
    Abstract base class for layers in tensor graph based probabilistic circuits.
    """

    index: Optional[int] = field(init=False, default=None)
    """
    The index this layer has in the tensor graph (self.probabilistic_circuit.graph)
    """

    probabilistic_circuit: Optional[ProbabilisticCircuit] = field(kw_only=True, default=None, repr=False)
    """
    The probabilistic circuit this layer belongs to.
    """

    result_of_current_query: Optional[Any] = field(init=False, default=None, repr=False)
    """
    The result of the current query during queries.
    """

    _reachable: Optional[np.array] = field(init=False, default=None, repr=False)
    """
    An array that signals if a node is reachable. This has the same length as self.nodes  
    """

    _valid: Optional[np.ndarray] = field(init=False, default=None, repr=False)
    """
    An array that signals if this node is still valid. This has the same length as self.nodes
    """
    ...
    def variables(self) -> SortedSet[Variable]:
        """
        :return: The set of variables in the layer.
        """
        raise NotImplementedError()
    ...
    @property
    def parent_layers(self) -> List[Layer]:
        return self.probabilistic_circuit.graph.predecessors(self.index)

    def update_reachable_nodes(self):
        """
        Update the reachable nodes of every child layer using the edge data.
        """
        raise NotImplementedError()
    ...
    def delete_unreachable_nodes(self):
        """
        Delete the unreachable nodes of this layer and update the edge data accordingly.
        Call this after calculating the reachable nodes with `update_reachable_nodes`.
        """
        raise NotImplementedError()
...
class InnerLayer(Layer, ABC):
    """
    Abstract base for inner layers.
    """
...
class SumLayer(InnerLayer):
    """
    Sum layers are a collection of sum nodes using convex weights.

    The weights of the layer are stored at the edges to child layers
    There are as many weight matrices as child layers.
    One of those weight matrices has shape (self.number_of_nodes, child_layer.number_of_nodes) and contains the
    logarithmic weights.
    ...
    def update_reachable_nodes(self):
        for log_weights, layer in self.log_weighted_child_layers:
            # update the own weights according to the reachability
            log_weights[~self._reachable, :] = -np.inf
            self.probabilistic_circuit.graph.add_edge(self.index, layer.index, log_weights)

            # calculate reachability of child layer nodes
            reachable = (log_weights > -np.inf).any(axis=0)
            assert reachable.shape == (layer.number_of_nodes,)
            layer._reachable |= reachable
...
class ProductLayer(InnerLayer):
    """
    A product layer is a collection of product nodes.

    The way the product layers connect to child layers is stored at the edges.
    It is represented by a boolean matrix of shape #self.number_of_nodes, child_layer.number_of_nodes)
    A value of true at index (i, j) means that node i of this layer is connected to node j of the child layer.
    """
    ...
    def edges(self) -> List[np.array]:
        ...
        The edge-matrix list has shape (#child_layers, #nodes, #child_layer_nodes)
...
class DiracDeltaLayer(ContinuousInputLayer):
    location: np.array
    """
    The locations of the dirac impulses
    """

    density_cap: np.array
    """
    The density cap of the dirac impulses
    """

    tolerance: Optional[np.array] = field(default=None)
    """
    The tolerance of the dirac impulses
    """
    ...
    def __deepcopy__(self, memo):
        return DiracDeltaLayer(self.variable, np.copy(self.location), np.copy(self.density_cap),
                               np.copy(self.tolerance))
    ...
    def delete_unreachable_nodes(self):
        self.location = self.location[self._reachable]
        self.density_cap = self.density_cap[self._reachable]
        self.tolerance = self.tolerance[self._reachable]
...
class ProbabilisticCircuit:
    ...
    def top_down_layers(self) -> List[Layer]:
        return [self.graph.get_node_data(i) for i in rx.topological_sort(self.graph)]
    ...
    def log_conditional_of_simple_event_in_place(self, simple_event: SimpleEvent) -> Tuple[Optional[Self], float]:
        ...
        # TODO remove all nodes that are not reachable or have no child nodes
    ...
    def remove_unreachable_nodes_inplace(self):
        """
        Removes all nodes that cannot be reached by any node of the root layer inplace.
        This assumes that the model is a tree.
        """

        # reset the reachable array
        for layer in self.layers:
            fill_value = layer == self.root
            layer._reachable = np.full(layer.number_of_nodes, fill_value, dtype=bool)

        # update reachability for all layers
        layers = self.top_down_layers()
        for layer in layers:
            layer.update_reachable_nodes()

        for layer in self.top_down_layers():
            layer.delete_unreachable_nodes()
