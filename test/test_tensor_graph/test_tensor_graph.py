...
class UnreachableNodesTestCase(unittest.TestCase):
    x = Continuous("x")
    model: ProbabilisticCircuit
    ...
    def setUpClass(cls):
        model = ProbabilisticCircuit()
        root = SumLayer(probabilistic_circuit=model)
        hidden = SumLayer(probabilistic_circuit=model)
        input_layer = DiracDeltaLayer(cls.x, np.array([0., 1.]), np.array([1, 2.,]),
                                      probabilistic_circuit=model)
        model.add_layer(root)
        model.add_layer(hidden)
        model.add_layer(input_layer)
        model.add_edge(root, hidden, np.log(np.array([[0.3, 0.7, 0], [0., 0., 0.]])))
        model.add_edge(hidden, input_layer, np.log(np.array([[1., 0.,], [1., 0.], [1., 0.]])))
        cls.model = model

    def test_remove_unreachable_nodes(self):
        model = copy.deepcopy(self.model)
        model.remove_unreachable_nodes_inplace()

        for layer in model.layers:
            if layer == model.root:
                self.assertEqual(layer.number_of_nodes, 2)
            elif isinstance(layer, DiracDeltaLayer):
                self.assertEqual(layer.number_of_nodes, 1)
                self.assertEqual(layer.location, np.array([0.]))
            else:
                self.assertEqual(layer.number_of_nodes, 2)
