...
class TruncatedGaussianDistributionTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.distribution = TruncatedGaussianDistribution(self.x, SimpleInterval(-2, 2, Bound.CLOSED, Bound.CLOSED),
                                                          2, 2.)
    ...
    def test_mode(self):
        mode, likelihood = self.distribution.univariate_log_mode()
        self.assertEqual(mode, singleton(2))
        self.assertEqual(likelihood, self.distribution.log_pdf(np.array([2])))
